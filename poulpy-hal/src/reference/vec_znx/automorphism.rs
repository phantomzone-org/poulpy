...
pub fn test_vec_znx_automorphism_inplace<B: Backend>(module: &Module<B>)
where
    Module<B>: VecZnxAutomorphismInplace<B> + VecZnxAutomorphismInplaceTmpBytes,
    ScratchOwned<B>: ScratchOwnedAlloc<B> + ScratchOwnedBorrow<B>,
{
    let mut source: Source = Source::new([0u8; 32]);
    let cols: usize = 2;

    let mut scratch: ScratchOwned<B> = ScratchOwned::alloc(module.vec_znx_automorphism_inplace_tmp_bytes());
