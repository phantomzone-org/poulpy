...
use crate::{
    api::{ModuleNew, VecZnxDftSub, VecZnxDftSubABInplace, VecZnxDftSubBAInplace},
...
pub fn test_vec_znx_dft_sub<B>(module: &Module<B>)
where
    Module<B>: VecZnxDftSub<B>,
...
pub fn test_vec_znx_dft_sub_ab_inplace<B>(module: &Module<B>)
where
    Module<B>: VecZnxDftSubABInplace<B>,
    B: Backend<ScalarPrep = f64> + VecZnxDftAllocBytesImpl<B>,
{
    let mut source: Source = Source::new([0u8; 32]);
    let cols: usize = 2;
...
pub fn test_vec_znx_dft_sub_ba_inplace<B>(module: &Module<B>)
where
    Module<B>: VecZnxDftSubBAInplace<B>,
    B: Backend<ScalarPrep = f64> + VecZnxDftAllocBytesImpl<B>,
{
    let mut source: Source = Source::new([0u8; 32]);
    let cols: usize = 2;
...
pub fn bench_vec_znx_dft_sub<B>(c: &mut Criterion, label: &str)
where
    Module<B>: VecZnxDftSub<B> + ModuleNew<B>,
...
pub fn bench_vec_znx_dft_sub_ab_inplace<B>(c: &mut Criterion, label: &str)
where
    Module<B>: VecZnxDftSubABInplace<B> + ModuleNew<B>,
    B: Backend + VecZnxDftAllocBytesImpl<B>,
{
...
pub fn bench_vec_znx_dft_sub_ba_inplace<B>(c: &mut Criterion, label: &str)
where
    Module<B>: VecZnxDftSubBAInplace<B> + ModuleNew<B>,
    B: Backend + VecZnxDftAllocBytesImpl<B>,
{
