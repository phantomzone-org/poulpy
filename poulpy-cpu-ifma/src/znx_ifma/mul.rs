...
pub unsafe fn znx_mul_power_of_two_ifma(k: i64, res: &mut [i64], a: &[i64]) {
...
/// [poulpy_hal::reference::znx::znx_mul_power_of_two_inplace_ref].
///
/// # Safety
/// Caller must ensure the CPU supports AVX-512F (e.g., via `is_x86_feature_detected!("avx512f")`);
/// all inputs must have the same length and must not alias.
...
/// all inputs must have the same length and must not alias.
#[target_feature(enable = "avx512f")]
pub unsafe fn znx_mul_add_power_of_two_ifma(k: i64, res: &mut [i64], a: &[i64]) {
    debug_assert_eq!(res.len(), a.len());
