macro_rules! hal_impl_convolution_ntt_ifma {
    () => {
        ...
        fn cnv_apply_dft_tmp_bytes(
            ...
            crate::ntt_ifma::convolution::cnv_apply_dft_ifma_tmp_bytes(a_size, b_size)
        }
            ...
            B: CnvPVecRToRef<Self>,
        {
            ...
            let bytes = crate::ntt_ifma::convolution::cnv_apply_dft_ifma_tmp_bytes(a.to_ref().size(), b.to_ref().size());
            let (tmp, _) = scratch.take_slice::<u8>(bytes);
            unsafe { crate::ntt_ifma::convolution::cnv_apply_dft_ifma(res, cnv_offset, res_col, a, a_col, b, b_col, tmp) }
        }
        ...
        {
            use poulpy_hal::api::TakeSlice;
            let bytes = crate::ntt_ifma::convolution::cnv_pairwise_apply_dft_ifma_tmp_bytes(
                res.to_mut().size(),
                a.to_ref().size(),
                b.to_ref().size(),
            );
            let (tmp, _) = scratch.take_slice::<u8>(bytes);
            unsafe { crate::ntt_ifma::convolution::cnv_pairwise_apply_dft_ifma(res, cnv_offset, res_col, a, b, i, j, tmp) }
        }
