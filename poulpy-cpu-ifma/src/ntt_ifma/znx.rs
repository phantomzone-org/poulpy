//! Single-ring coefficient-domain arithmetic for [`NTTIfma`](crate::NTTIfma).
//!
//! These operations work on plain `&[i64]` slices and are intentionally shared with
//! the portable backend. This module therefore acts mainly as trait wiring from the
//! IFMA backend type to the backend-independent reference implementations.
