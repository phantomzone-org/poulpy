//! Scratch-buffer integration for [`NTTIfma`](crate::NTTIfma).
//!
//! The scratch implementation is shared with the portable IFMA reference path:
//! it provides 64-byte-aligned owned buffers, borrowing as `&mut Scratch<B>`,
//! arena-style sub-allocation through `take_slice`, and available-space queries.
...
unsafe impl<B: Backend> ScratchOwnedAllocImpl<B> for NTTIfma {
...
unsafe impl<B: Backend> ScratchOwnedBorrowImpl<B> for NTTIfma
where
...
unsafe impl<B: Backend> ScratchFromBytesImpl<B> for NTTIfma {
...
unsafe impl<B: Backend> ScratchAvailableImpl<B> for NTTIfma {
...
unsafe impl<B: Backend> TakeSliceImpl<B> for NTTIfma
where
    B: ScratchFromBytesImpl<B>,
{
