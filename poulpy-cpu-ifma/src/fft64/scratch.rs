...
unsafe impl<B: Backend> ScratchOwnedAllocImpl<B> for FFT64Ifma {
...
unsafe impl<B: Backend> ScratchOwnedBorrowImpl<B> for FFT64Ifma
where
...
unsafe impl<B: Backend> ScratchFromBytesImpl<B> for FFT64Ifma {
    fn scratch_from_bytes_impl(data: &mut [u8]) -> &mut Scratch<B> {
        unsafe { &mut *(data as *mut [u8] as *mut Scratch<B>) }
    }
}

unsafe impl<B: Backend> ScratchAvailableImpl<B> for FFT64Ifma {
...
unsafe impl<B: Backend> TakeSliceImpl<B> for FFT64Ifma
where
    B: ScratchFromBytesImpl<B>,
{
