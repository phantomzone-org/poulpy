...
/// # Thread safety
///
/// `FFT64Ifma` is `Send + Sync` (derived from being a zero-sized, field-less struct).
/// The `Module<FFT64Ifma>` that holds the FFT tables is also `Send + Sync`, so modules can
...
///
/// let module: Module<FFT64Ifma> = Module::new(1024);
/// // Use module for FHE operations...
/// ```
pub struct FFT64Ifma {}
