...
use poulpy_schemes::tfhe::{
    bdd_arithmetic::{
        ...
        FheUintPrepareDebug, FheUintPrepared, FheUintPreparedDebug, FheUintPreparedEncryptSk, FheUintPreparedFactory,
        tests::test_suite::{TestContext},
    },
    blind_rotation::{BlindRotationAlgo, BlindRotationKey, BlindRotationKeyFactory, CGGI},
};

pub fn benc_bdd_prepare<BRA: BlindRotationAlgo + Sync, BE: Backend + Sync>(c: &mut Criterion, label: &str, test_context: &TestContext<BRA, BE>)
where
    ...
    Scratch<BE>: ScratchTakeCore<BE>,
{
    ...
    fn runner<BE: Backend + Sync, BRA: BlindRotationAlgo + Sync>(test_context: &TestContext<BRA, BE>) -> impl FnMut()
    where
        Module<BE>: ModuleNew<BE>
            + GLWESecretPreparedFactory<BE>
            + GLWEDecrypt<BE>
        ...
        Scratch<BE>: ScratchTakeCore<BE>,
    {
        let glwe_infos: GLWELayout = test_context.get_glwe_infos();
        let ggsw_infos: GGSWLayout = test_context.get_ggsw_infos();
...
fn bench_bdd_prepare_cpu_ref_fft64(c: &mut Criterion) {
    benc_bdd_prepare::<CGGI, FFT64Ref>(
        c,
        "bdd_prepare_fft64_ref",
        &TestContext::<CGGI, FFT64Ref>::new(),
    );
}
