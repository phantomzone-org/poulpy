...
pub fn vec_znx_mul_xp_minus_one<'r, 'a, BE>(
    ...
    BE: Backend + ZnxRotate + ZnxZero + ZnxSubInplace,
...
pub fn vec_znx_mul_xp_minus_one_inplace<'r, BE>(p: i64, res: &mut VecZnxBackendMut<'r, BE>, res_col: usize, tmp: &mut [i64])
where
    BE: Backend + ZnxRotate + ZnxNegate + ZnxSubNegateInplace,
    BE::BufMut<'r>: HostDataMut,
{
    ...
    for j in 0..res.size() {
        ...
        BE::znx_sub_negate_inplace(res.at_mut(res_col, j), tmp);
    }
}
