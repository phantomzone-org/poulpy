use backend::hal::{
    api::{
        ScratchAvailable, TakeVecZnx, TakeVecZnxBig, TakeVecZnxDft, VecZnxAllocBytes, VecZnxBigAllocBytes, VecZnxCopy,
        VecZnxDftAddInplace, VecZnxDftCopy, VecZnxDftToVecZnxBigTmpA, VecZnxNormalizeTmpBytes, ZnxInfos,
    },
    layouts::{Backend, DataMut, DataRef, Module, Scratch, VmpPMat},
};
...
impl<DataSelf: DataMut> GGSWCiphertext<DataSelf> {
    pub fn from_gglwe<DataA, DataTsk, B: Backend>(
        ...
        Scratch<B>: TakeVecZnxDft<B> + TakeVecZnxBig<B> + ScratchAvailable + TakeVecZnx<B>,
    {
        ...
        Scratch<B>: TakeVecZnxDft<B> + TakeVecZnxBig<B> + ScratchAvailable,
    {
        ...
        (0..self.rows()).for_each(|row_i| {
            ...
            (1..cols).for_each(|col_j| {
                ...
                let (mut tmp_dft_i, scratch2) = scratch1.take_vec_znx_dft(module, cols, tsk.size());
                let (mut tmp_a, scratch3) = scratch2.take_vec_znx_dft(module, 1, ci_dft.size().div_ceil(digits));
                ...
                let (mut tmp_idft, scratch3) = scratch2.take_vec_znx_big(module, 1, tsk.size());
                (0..cols).for_each(|i| {
