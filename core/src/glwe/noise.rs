...
impl<D: AsRef<[u8]>> GLWECiphertext<D>{
    ...
    pub fn assert_noise<B: Backend, DataSk, DataPt>(&self, module: &Module<B>, sk_exec: &GLWESecretExec<DataSk, B>, pt_want: &GLWEPlaintext<DataPt>, max_noise: f64) where DataSk: AsRef<[u8]>, DataPt: AsRef<[u8]>, Module<B>: GLWEDecryptFamily<B>{

        let mut pt_have: GLWEPlaintext<Vec<u8>> = GLWEPlaintext::alloc(module, self.basek(), self.k());

        let mut scratch: ScratchOwned = ScratchOwned::new(GLWECiphertext::decrypt_scratch_space(module, self.basek(), self.k()));
