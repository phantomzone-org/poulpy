...
use backend::hal::{
    api::{
        MatZnxAlloc, ScalarZnxAlloc, ScalarZnxAllocBytes, ScalarZnxAutomorphism, ScratchAvailable, TakeMatZnx, TakeScalarZnx,
        TakeSvpPPol, TakeVecZnx, TakeVecZnxBig, TakeVecZnxDft, TakeVecZnxDftSlice, TakeVecZnxSlice, VecZnxAddInplace,
        VecZnxAddScalarInplace, VecZnxAlloc, VecZnxAllocBytes, VecZnxAutomorphismInplace, VecZnxBigAutomorphismInplace,
        VecZnxBigSubSmallBInplace, VecZnxCopy, VecZnxDftCopy, VecZnxDftToVecZnxBigTmpA, VecZnxNegateInplace,
        VecZnxNormalizeInplace, VecZnxNormalizeTmpBytes, VecZnxRotateInplace, VecZnxRshInplace, VecZnxSub, VecZnxSubABInplace,
        VecZnxSwithcDegree,
    },
...
impl CircuitBootstrappingKeyCGGI<Vec<u8>> {
    ...
    where
        ...
        DLwe: DataRef,
        DGlwe: DataRef,
        Scratch<B>: TakeVecZnxDft<B> + ScratchAvailable + TakeVecZnx<B> + TakeScalarZnx<B> + TakeSvpPPol<B> + TakeVecZnxBig<B>,
    {
        let mut auto_keys: HashMap<i64, AutomorphismKey<Vec<u8>>> = HashMap::new();
        let gal_els: Vec<i64> = GLWECiphertext::trace_galois_elements(&module);
        gal_els.iter().for_each(|gal_el| {
            let mut key: AutomorphismKey<Vec<u8>> = AutomorphismKey::alloc(module, basek, k_trace, rows_trace, 1, sk_glwe.rank());
            key.encrypt_sk(
        ...
        let mut brk: BlindRotationKeyCGGI<Vec<u8>> =
            BlindRotationKeyCGGI::alloc(module, sk_lwe.n(), basek, k_brk, rows_brk, sk_glwe.rank());
        ...
        let mut tsk: GLWETensorKey<Vec<u8>> = GLWETensorKey::alloc(module, basek, k_tsk, rows_tsk, 1, sk_glwe.rank());
...
pub trait CGGICircuitBootstrapFamily<B: Backend> = VecZnxAlloc
    + VecZnxRotateInplace
        ...
        + TakeVecZnxSlice<B>,
{
    ...
    let (mut res_glwe, scratch1) = scratch.take_glwe_ct(module, basek, k, rank);
    let (mut tmp_gglwe, scratch2) = scratch1.take_gglwe(module, basek, k, rows, 1, rank, rank);
...
pub fn pack<D: DataMut, B: Backend>(
    ...
    Module<B>: CGGICircuitBootstrapFamily<B>,
    Scratch<B>: TakeVecZnx<B> + TakeVecZnxDft<B> + ScratchAvailable,
{
    let log_n: usize = module.log_n();
