...
use poulpy_hal::{
    ...
    layouts::{Backend, HostDataMut, Module, ScalarZnx, ScratchArena, ScratchOwned, ZnxViewMut},
    source::Source,
};
use rand::Rng;
...
pub fn test_glwe_blind_retrieval_statefull<BRA: BlindRotationAlgo, BE: Backend<OwnedBuf = Vec<u8>>>(
...
pub fn test_glwe_blind_retriever<BRA: BlindRotationAlgo, BE: Backend<OwnedBuf = Vec<u8>>>(test_context: &TestContext<BRA, BE>)
where
    Module<BE>: GLWEEncryptSk<BE> + GLWEDecrypt<BE> + GLWEBlindRetrieval<BE> + GGSWEncryptSk<BE> + GGSWPreparedFactory<BE>,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    for<'a> ScratchArena<'a, BE>: poulpy_core::ScratchArenaTakeCore<'a, BE>,
