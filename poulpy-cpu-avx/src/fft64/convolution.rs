    ...
    Module<Self>: ModuleN + VecZnxDftBytesOf + VecZnxDftApply<Self>,
{
    ...
    fn cnv_prepare_self_tmp_bytes_impl(module: &Module<Self>, res_size: usize, a_size: usize) -> usize {
        module.bytes_of_vec_znx_dft(1, res_size.min(a_size))
    }
    ...
    {
        let left: &mut CnvPVecL<&mut [u8], FFT64Avx> = &mut left.to_mut();
        let right: &mut CnvPVecR<&mut [u8], FFT64Avx> = &mut right.to_mut();
        let a: &VecZnx<&[u8]> = &a.to_ref();
