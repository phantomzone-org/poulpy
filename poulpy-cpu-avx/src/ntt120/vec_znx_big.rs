...
use super::{
    NTT120Avx,
    vec_znx_big_avx::{
        nfc_final_step_assign_avx2, nfc_final_step_assign_scalar, nfc_final_step_inplace_avx2, nfc_final_step_inplace_scalar,
        nfc_middle_step_assign_avx2, nfc_middle_step_assign_scalar, nfc_middle_step_avx2, nfc_middle_step_inplace_avx2,
...
impl I128NormalizeOps for NTT120Avx {
    ...
    fn nfc_middle_step_assign<O: AssignOp>(base2k: usize, lsh: usize, res: &mut [i64], a: &[i128], carry: &mut [i128]) {
        if base2k <= 64 && res.len() >= 4 {
            unsafe { nfc_middle_step_assign_avx2::<O>(base2k as u32, lsh as u32, res.len(), res, a, carry) }
        } else {
            nfc_middle_step_assign_scalar::<O>(base2k, lsh, res, a, carry);
        }
    }
    ...
    fn nfc_final_step_assign<O: AssignOp>(base2k: usize, lsh: usize, res: &mut [i64], carry: &mut [i128]) {
        if base2k <= 64 && res.len() >= 4 {
            unsafe { nfc_final_step_assign_avx2::<O>(base2k as u32, lsh as u32, res.len(), res, carry) }
        } else {
            nfc_final_step_assign_scalar::<O>(base2k, lsh, res, carry);
        }
    }
}
