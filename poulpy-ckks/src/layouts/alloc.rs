...
use super::{CKKSCiphertext, CKKSPlaintextVecZnx};
...
pub trait CKKSModuleAlloc<BE: Backend>: ModuleCoreAlloc<OwnedBuf = BE::OwnedBuf> {
    ...
    fn ckks_pt_vec_znx_alloc_from_infos<A>(&self, infos: &A) -> CKKSPlaintextVecZnx<BE::OwnedBuf>
    where
        A: GLWEInfos + CKKSInfos;

    fn ckks_pt_vec_znx_alloc(&self, base2k: Base2K, meta: CKKSMeta) -> CKKSPlaintextVecZnx<BE::OwnedBuf>;
}
    ...
    Module<BE>: ModuleCoreAlloc<OwnedBuf = BE::OwnedBuf>,
{
    ...
    fn ckks_pt_vec_znx_alloc(&self, base2k: Base2K, meta: CKKSMeta) -> CKKSPlaintextVecZnx<BE::OwnedBuf> {
        CKKSPlaintextVecZnx::from_inner(self.glwe_plaintext_alloc(base2k, meta.min_k(base2k)), meta)
    }
}
