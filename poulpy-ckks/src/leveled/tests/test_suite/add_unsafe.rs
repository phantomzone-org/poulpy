...
pub fn test_add_pt_vec_rnx_into_aligned_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_add_pt_vec_rnx_into_unsafe(&mut ct_res, &ct1, &pt_rnx, ctx.meta(), &mut scratch.borrow())
            .unwrap();
    }
    ...
    ctx.assert_decrypt_precision("add_pt_vec_rnx_unsafe", &ct_res, &want_re, &want_im, &mut scratch.borrow());
}
...
pub fn test_add_const_znx_into_aligned_unsafe<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    unsafe {
        ctx.module
            .ckks_add_pt_const_znx_into_unsafe(&mut ct_res, &ct, &cst_znx, &mut scratch.borrow())
            .unwrap();
    }
