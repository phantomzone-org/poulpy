...
pub fn test_mul_pt_znx_aligned<BE: Backend>(ctx: &TestContext<BE>) {
    ...
    let (want_re, want_im) = ctx.want_mul();
    let mut ct_res = ctx.alloc_ct(ctx.max_k());
    ctx.module.ckks_mul_pt_znx(&mut ct_res, &ct, &pt, scratch.borrow()).unwrap();
...
pub fn test_mul_pt_znx_inplace<BE: Backend>(ctx: &TestContext<BE>) {
    ...
    let (want_re, want_im) = ctx.want_mul();
    let ct_meta = ct.meta();
    ctx.module.ckks_mul_pt_znx_inplace(&mut ct, &pt, scratch.borrow()).unwrap();
...
pub fn test_mul_pt_rnx_aligned<BE: Backend>(ctx: &TestContext<BE>) {
    ...
    let mut ct_res = ctx.alloc_ct(ctx.max_k());
    ctx.module
        .ckks_mul_pt_rnx(&mut ct_res, &ct, &pt, ctx.meta(), scratch.borrow())
        .unwrap();
...
pub fn test_mul_pt_rnx_delta_log_decimal<BE: Backend>(ctx: &TestContext<BE>) {
    ...
    let mut ct_res = ctx.alloc_ct(ctx.max_k());
    ctx.module
        .ckks_mul_pt_rnx(&mut ct_res, &ct, &pt, low_prec, scratch.borrow())
        .unwrap();
...
pub fn test_mul_pt_rnx_inplace<BE: Backend>(ctx: &TestContext<BE>) {
    ...
    let ct_meta = ct.meta();
    ctx.module
        .ckks_mul_pt_rnx_inplace(&mut ct, &pt, ctx.meta(), scratch.borrow())
        .unwrap();
