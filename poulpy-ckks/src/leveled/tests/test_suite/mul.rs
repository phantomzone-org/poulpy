...
/// Verifies ct × ct multiplication when the operands use different `log_delta`.
///
/// The result scale should be `max(log_delta_a, log_delta_b)`, and rescale
/// should consume `min(log_delta_a, log_delta_b)` bits from `k`.
