...
//! | [`test_mul_add_pt_vec_znx_delta_log_decimal`] | ZNX plaintext at lower `log_decimal` |
...
use crate::{CKKSInfos, layouts::plaintext::CKKSConstPlaintextConversion, leveled::operations::composite::CKKSMulAddOps};
...
pub fn test_mul_add_pt_vec_znx_delta_log_decimal<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    let mut scratch = alloc_scratch(ctx);
    let half = F::from_f64(0.5).unwrap();
...
pub fn test_mul_add_const_rnx_zero_preserves_dst_meta<BE: Backend, F: TestScalar>(ctx: &TestContext<BE, F>) {
    ...
    ctx.module
        .ckks_mul_add_const_rnx(&mut dst, &a, &cst, ctx.precision_at(ctx.meta().log_decimal - DELTA_LOG_DECIMAL), scratch.borrow())
        .unwrap();
