...
use crate::{
    ...
    leveled::operations::sub::{sub, sub_const, sub_const_inplace, sub_inplace, sub_pt, sub_pt_inplace},
};
use poulpy_core::{
...
pub fn test_sub<BE: Backend>(module: &Module<BE>, params: CKKSTestParams)
where
    ...
    Scratch<BE>: ScratchTakeCore<BE>,
{
    ...
    sub_inplace(module, &mut ct_ip, &ct2);
...
pub fn test_sub_pt<BE: Backend>(module: &Module<BE>, params: CKKSTestParams)
where
    Module<BE>: ModuleN + GLWEEncryptSk<BE> + GLWEDecrypt<BE> + GLWESecretPreparedFactory<BE> + GLWEAdd + GLWESub,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    Scratch<BE>: ScratchTakeCore<BE>,
{
    ...
    let mut ct_res = CKKSCiphertext::alloc(Degree(n as u32), base2k, k, params.log_delta);
    sub_pt(module, &mut ct_res, &ct1, &pt2);
    let (re_out, im_out) = decrypt_decode(module, params, &ct_res, &sk, &mut scratch_be);
    ...
    sub_pt_inplace(module, &mut ct_ip, &pt2);
    let (re_ip, im_ip) = decrypt_decode(module, params, &ct_ip, &sk, &mut scratch_be);
    for j in 0..m {
        assert!((re_ip[j] - (re1[j] - re2[j])).abs() < t, "inplace re[{j}]");
        assert!((im_ip[j] - (im1[j] - im2[j])).abs() < t, "inplace im[{j}]");
...
pub fn test_sub_const<BE: Backend>(module: &Module<BE>, params: CKKSTestParams)
where
    Module<BE>: ModuleN + GLWEEncryptSk<BE> + GLWEDecrypt<BE> + GLWESecretPreparedFactory<BE> + GLWEAdd + GLWESub,
    ScratchOwned<BE>: ScratchOwnedAlloc<BE> + ScratchOwnedBorrow<BE>,
    Scratch<BE>: ScratchTakeCore<BE>,
{
    ...
    let k = TorusPrecision(params.k);
    let mut ct_res = CKKSCiphertext::alloc(Degree(n as u32), base2k, k, params.log_delta);
    sub_const(module, &mut ct_res, &ct1, c_re, c_im);
    let (re_out, im_out) = decrypt_decode(module, params, &ct_res, &sk, &mut scratch_be);
    ...
    sub_const_inplace(module, &mut ct_ip, c_re, c_im);
    let (re_ip, im_ip) = decrypt_decode(module, params, &ct_ip, &sk, &mut scratch_be);
    for j in 0..m {
