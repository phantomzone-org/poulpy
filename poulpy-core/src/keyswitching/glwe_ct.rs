...
use crate::layouts::{GGLWEMetadata, GLWECiphertext, GLWEMetadata, Infos, prepared::GGLWESwitchingKeyPrepared};
...
impl GLWECiphertext<Vec<u8>> {
    ...
    {
        let in_size: usize = in_metadata
            .k
            .div_ceil(key_metadata.basek)
            .div_ceil(key_metadata.digits);
        let out_size: usize = out_metadata.k.div_ceil(out_metadata.basek);
        let ksk_size: usize = key_metadata.k.div_ceil(key_metadata.basek);
        let res_dft: usize = module.vec_znx_dft_alloc_bytes(key_metadata.rank_out + 1, ksk_size); // TODO OPTIMIZE
        let ai_dft: usize = module.vec_znx_dft_alloc_bytes(key_metadata.rank_in, in_size);
        let vmp: usize = module.vmp_apply_dft_to_dft_tmp_bytes(
            out_size,
            in_size,
            in_size,
            key_metadata.rank_in,
            key_metadata.rank_out + 1,
            ksk_size,
        ) + module.vec_znx_dft_alloc_bytes(key_metadata.rank_in, in_size);
        ...
        if in_metadata.basek == key_metadata.basek {
            ...
            if key_metadata.digits == 1 {
                ...
                let normalize_conv: usize = VecZnx::alloc_bytes(module.n(), key_metadata.rank_in, in_size);
    ...
    where
        Module<B>: VecZnxDftAllocBytes + VmpApplyDftToDftTmpBytes + VecZnxBigNormalizeTmpBytes + VecZnxNormalizeTmpBytes,
    {
        Self::keyswitch_scratch_space(module, out_metadata, out_metadata, key_metadata)
    }
}
...
impl<DataSelf: DataRef> GLWECiphertext<DataSelf> {
        ...
        Scratch<B>: ScratchAvailable,
    {
        ...
        let scrach_needed = GLWECiphertext::keyswitch_scratch_space(module, self.metadata, lhs.metadata, rhs.metadata());
        ...
        Scratch<B>: ScratchAvailable,
    {
        ...
        let scrach_needed = GLWECiphertext::keyswitch_inplace_scratch_space(module, self.metadata, rhs.metadata());
